...
const shouldOfferUpdateForTrack = (latestVersion: string, currentVersion: string): boolean => {
  return isRemoteVersionNewer(latestVersion, currentVersion)
}
...
const getStableUpdaterFeedUrl = (): string => {
  const repoBase = 'https://github.com/Ysr541/SalesAssistant/releases'
  return `${repoBase}/latest/download`
}

const applyAutoUpdateChannel = () => {
  const nextFeedUrl = getStableUpdaterFeedUrl()
...
const collectLegacySnsCacheMigrationPlan = async (): Promise<SnsCacheMigrationPlan | null> => {
  ...
  let currentBaseDir = configuredCachePath
  if (!currentBaseDir) {
    try {
      currentBaseDir = join(app.getPath('documents'), 'WeFlow')
    } catch {
      currentBaseDir = join(app.getPath('userData'), 'cache')
    }
  }
...
function registerIpcHandlers() {
  ...
  ipcMain.handle('chat:clearCurrentAccountData', async (_, options?: { clearCache?: boolean; clearExports?: boolean }) => {
    ...
    if (clearCache) {
      ...
      let documentsWeFlowDir: string
      try {
        documentsWeFlowDir = join(app.getPath('documents'), 'WeFlow')
      } catch {
        documentsWeFlowDir = join(app.getPath('userData'), 'cache')
      }
  ...
  ipcMain.handle('export:exportSessions', async (event, sessionIds: string[], outputDir: string, options: ExportOptions, controlOptions?: { taskId?: string }) => {
    ...
    let emojiCacheDir: string
    if (cachePath) {
      emojiCacheDir = join(cachePath, 'Emojis')
    } else {
      try {
        emojiCacheDir = join(app.getPath('documents'), 'WeFlow', 'Emojis')
      } catch {
        emojiCacheDir = join(app.getPath('userData'), 'Emojis')
      }
    }
  ...
  ipcMain.handle('export:exportSession', async (event, sessionId: string, outputPath: string, options: ExportOptions) => {
    ...
    let singleEmojiCacheDir: string
    if (singleCachePath) {
      singleEmojiCacheDir = join(singleCachePath, 'Emojis')
    } else {
      try {
        singleEmojiCacheDir = join(app.getPath('documents'), 'WeFlow', 'Emojis')
      } catch {
        singleEmojiCacheDir = join(app.getPath('userData'), 'Emojis')
      }
    }
    const workerPath = join(__dirname, 'exportWorker.js')
  ...
  ipcMain.handle('whisper:downloadModel', async (event, modelDir?: string) => {
    return voiceTranscribeService.downloadModel((progress) => {
      event.sender.send('whisper:downloadProgress', progress)
    }, modelDir)
  })

  ipcMain.handle('whisper:getModelStatus', async (_, modelDir?: string) => {
    return voiceTranscribeService.getModelStatus(modelDir)
  })
