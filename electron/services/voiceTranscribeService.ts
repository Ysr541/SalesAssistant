...
export class VoiceTranscribeService {
  ...
  private resolveModelDir(customDir?: string): string {
  ...
  private resolveModelPath(fileName: string, customDir?: string): string {
    return join(this.resolveModelDir(customDir), fileName)
  }

  /**
   * 检查模型状态
   */
  async getModelStatus(modelDir?: string): Promise<{
    success: boolean
    exists?: boolean
    modelPath?: string
    tokensPath?: string
    ...
    try {
      const modelPath = this.resolveModelPath(SENSEVOICE_MODEL.files.model, modelDir)
      const tokensPath = this.resolveModelPath(SENSEVOICE_MODEL.files.tokens, modelDir)
  ...
  async downloadModel(
    ...
    const task = (async () => {
      try {
        ...
        const modelPath = this.resolveModelPath(SENSEVOICE_MODEL.files.model, customModelDir)
        const tokensPath = this.resolveModelPath(SENSEVOICE_MODEL.files.tokens, customModelDir)
