...
export class ConfigService {
  ...
  private migrateAiConfig(): void {
    ...
    // 自动调整 maxTokens 配置（超过 2048 的值全部重置为 1600，1024 也升级为 1600）
    const currentMaxTokens = this.get('aiModelApiMaxTokens')
    if (currentMaxTokens === 1024 || currentMaxTokens > 2048) {
      this.set('aiModelApiMaxTokens', 1600)
    }
  }
